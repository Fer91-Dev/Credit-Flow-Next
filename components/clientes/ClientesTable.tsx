...
import { Plus, Trash2, Edit2, Mail, Phone, Users, UserCheck, PhoneOff, UserPlus, Search, ChevronDown, X } from "lucide-react";
...
export function ClientesTable() {
  ...
  const filtered = useMemo(() => {
    ...
    return clientes.filter(c =>
      (!q || c.nombre.toLowerCase().includes(q) || (c.documento || "").toLowerCase().includes(q)) &&
                ...
                <tbody>
                  {filtered.map((c, idx) => (
                    <tr key={c.id} className={`hover:bg-muted/20 transition-colors ${idx % 2 === 1 ? "bg-muted/5" : ""}`}>
                      <td className="px-4 py-3 font-medium text-foreground border-b border-border/40">{c.nombre}</td>
              ...
              {filtered.map(c => (
                <div key={c.id} className="rounded-xl bg-card border border-border p-4 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <p className="font-medium text-foreground text-sm leading-tight">{c.nombre}</p>
                    <StatusBadge label={c.estado} variant={c.estado === "activo" ? "success" : "muted"} />
                  </div>
                  {c.documento && (
